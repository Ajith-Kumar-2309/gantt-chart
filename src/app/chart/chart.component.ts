...
export class ChartDatabase {
  id : any; // chart id
  moment = moment;
  dataChange = new BehaviorSubject<any>(null);
  storageKey = 'charts';
  ...
  saveStorage(val:any) {
    ...
    const charts :any = JSON.parse(local) as Array<Step>;
    charts[this.id] = val;
...
export class ChartComponent {
  ...
  deleteStep(node: StepFlatNode) {
    ...
    const parentFlatNode : any = this.getParentStep(node);
    const parentNode : any = this.flatNodeMap.get(parentFlatNode);
    const childNode : any = this.flatNodeMap.get(node);
  ...
  updateProgress(node: StepFlatNode, progress: number) {
    const nestedNode : any = this.flatNodeMap.get(node);
    const newProgressDates : any = this.database.updateProgress(nestedNode, progress);
